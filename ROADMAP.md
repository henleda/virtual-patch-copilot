...
- [ ] **G4** Published model scorecard. (M, P1)
  ...
  - Acceptance: `vpcopilot bench <repo> --all-configs` writes `benchmarks/RESULTS.md` with one
    ...
    accuracy, bonus finds, noise, wall time, and token cost; re-running with the same seed and
    model reproduces the score; `MODELS.md` links the table rather than restating numbers.
